...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        source_path = image_path.split("/")[-1]
        filename = os.path.basename(source_path)
        destination_directory = os.path.dirname(filename.split('_')[1])
        destination_path = os.path.join(savefolder, destination_directory)
        ...
        f.tight_layout()
        f.savefig(destination_file_path)
        plt.close()

...
class SimpleNet(torch.nn.Module):
    def __init__(self, device):
        ...
        self.device = device
        print(f'self_device = {self.device}')
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt, save=False, epoch=0):
        ...
        if len(masks_gt) > 0:
            ...
            if save:
                self.save_segmentation_images(test_data, segmentations, scores, epoch)
    ...
    def train(self, training_data, test_data):
        ...
        for i_mepoch in range(self.meta_epochs):
            ...
            scores, segmentations, features, labels_gt, masks_gt = self.predict(test_data)
            if (i_mepoch % 25 == 0):
                auroc, full_pixel_auroc, pro = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt, save=True, epoch=i_mepoch)
            else:
    ...
    def _train_discriminator(self, input_data):
            ...
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    if self.pre_proj > 0:
                        ...
                        times_per_img.append(end-start)
                    else:
                        start = time.time()
        ...
        print('================================================')
        print(f'MEAN TIME PER IMAGE: {(sum(times_per_img)/len(times_per_img) * 1000)}')
        print('================================================')
    ...
    def _predict_dataloader(self, dataloader, prefix):
        ...
        with tqdm.tqdm(dataloader, desc="Inferring...", leave=False) as data_iterator:
            for data in data_iterator:
                ...
                total_images += len(image)  # Count the number of images in this batch

        # Calculate performance metrics
        mean_latency = (total_time / total_images) * 1000  # ms per image
        mean_ips = total_images / total_time  # images per second

        # Print the metrics at the end
        print(f"\nPerformance Metrics:")
        print(f"Mean Latency per Image: {mean_latency:.2f} ms")
        print(f"Mean Images per Second: {mean_ips:.2f}")
    ...
    def save_segmentation_images(self, data, segmentations, scores, epoch=0):
        image_paths = [

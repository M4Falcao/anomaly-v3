"""Evaluate a single trained model and plot its ROC / score distributions.

Loads a model saved under ``models/`` via ``config.modelname``, scores the test
split of ``config.class_name`` and reports AUROC, accuracy and the confusion
matrix at the best threshold.

Usage:
    python scripts/eval/evaluate_model_roc.py
"""
...
import config as c
from core.model import load_model, SEDifferNet
from core.utils import load_datasets, make_dataloaders, t2np

def evaluate_model(model, test_loader):
    """Score every test image and return the ground-truth labels and scores.

    Args:
        model: Trained model to evaluate.
        test_loader: Dataloader yielding ``(images, labels)`` with images shaped
            ``(batch, n_transforms, channels, height, width)``.

    Returns:
        Tuple ``(y_true, y_scores)`` of numpy arrays.
    """
    model.eval()

...
### `score_image_folder.py`
- **Para que serve:** Realiza inferência com um modelo treinado sobre uma pasta de imagens, imprimindo um score de anomalia por imagem (com test-time augmentation por rotações fixas). Não calcula métricas — apenas pontuação.
- **Como usar:** `python scripts/eval/score_image_folder.py`
...
### `evaluate_cam_xai_metrics.py`
- **Para que serve:** Avalia a fidelidade das explicações visuais (GradCAM) do modelo, calculando as métricas Deletion, Insertion e ROAD (RemOve And Debias). Opcionalmente treina/avalia a cabeça de pixel CFLOW e o PatchCore na mesma execução e pode disparar o sanity check.
- **Como usar:** `python scripts/eval/evaluate_cam_xai_metrics.py --model_path <caminho.pt> --class_name <classe>`

### `evaluate_cam_sanity_check.py`
...
- **Como usar:** `python scripts/eval/evaluate_cam_sanity_check.py --model_path <caminho.pt> --class_name <classe>`

### `test_model_grad_maps.py`
- **Para que serve:** Testa um checkpoint salvo e reporta métricas em nível de imagem e de pixel, derivando os mapas de anomalia a partir dos gradientes da entrada. Muito útil para validações manuais diretas via CLI.
- **Como usar:** `python scripts/eval/test_model_grad_maps.py --model_path <caminho.pt> --dataset_path <caminho> --class_name <classe>`

### `test_single_checkpoint.py`
...
- **Como usar:** `python scripts/eval/test_single_checkpoint.py --model_group cbamdiffernet --class_name transistor`
...
- **Como usar:** `python scripts/eval/sweep_checkpoints.py --models_dir ./checkpoints/ --output_file resultados.txt`

## 4. Análise e Visualização

### `evaluate_model_roc.py`
- **Para que serve:** Executa uma inferência no conjunto de testes e gera gráficos KDE e histogramas comparando as distribuições de scores das imagens "Normais" e das imagens com "Anomalias", além da curva ROC e da matriz de confusão. Ajuda a entender a precisão do modelo e determinar visualmente o limiar (threshold) de corte ótimo.
- **Como usar:** `python scripts/eval/evaluate_model_roc.py --model_path <caminho.pt>`

### `visualize_cam_metric_steps.py`
...
- **Como usar:** `python scripts/eval/visualize_cam_metric_steps.py --model_path <caminho.pt> --index 0 --steps 5`
...
- **Como usar:** `python scripts/tools/print_output_log.py`

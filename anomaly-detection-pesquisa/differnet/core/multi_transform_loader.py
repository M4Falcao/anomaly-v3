"""``ImageFolder`` variant that returns several transforms of the same image.

DifferNet scores an image by averaging over multiple augmented views of it, so
the dataset must yield a stack of transformed copies per sample rather than a
single tensor.

Two modes are supported:

- Random transforms (training): ``config.n_transforms`` random augmentations.
- Fixed rotations (testing): ``config.n_transforms_test`` evenly spaced angles,
  which makes evaluation deterministic.
"""
...
def fixed_rotation(self, sample, degrees):
    """Build the transform pipeline for one fixed rotation angle.

    Args:
        sample: PIL image to transform.
        degrees: Rotation angle in degrees.

    Returns:
        The transformed tensor for that angle.
    """

"""Anomaly localization by back-propagating the loss to the input pixels.

DifferNet has no pixel-level output head. To localize a defect, the gradient of
the normalizing-flow loss with respect to the input image is used as a saliency
map: pixels that most increase the negative log-likelihood are the most
anomalous.

Maps are un-rotated back to the original frame, averaged over all test-time
rotations, smoothed, and written to ``gradient_maps/<config.modelname>/``.
"""
...
GRADIENT_MAP_DIR = project_path('gradient_maps')

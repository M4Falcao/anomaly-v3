...
class SEBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.se = SEBlock(planes * self.expansion)  # SE block after penultimate layer
        self.relu = nn.ReLU(inplace=True)
...
class CBAMBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(planes * self.expansion)
        self.cbam = CBAM(planes * self.expansion)  # CBAM block after penultimate layer
        self.relu = nn.ReLU(inplace=True)
        self.upsample = upsample
        self.stride = stride
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.cbam(out)  # Apply CBAM here
...
def de_wide_resnet50_2_senet(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
...
def de_wide_resnet50_2_cbam(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    r"""Wide ResNet-50-2 model from
    `"Wide Residual Networks" <https://arxiv.org/pdf/1605.07146.pdf>`_.
    The model is the same as ResNet except for the bottleneck number of channels
    which is twice larger in every block. The number of channels in outer 1x1
